...
ports. The next architectural risk is that `@eregister/wit-grid-core/internal` is still a
...
After this plan, `@eregister/wit-grid-core/internal` should read like a deliberate
...
- `packages/react/src/WitGrid.tsx` imports both the host API and
  `InternalColumnDef` from the broad internal barrel:

```ts
// packages/react/src/WitGrid.tsx:16
import { InternalColumnDef, GridHostWithAdapter, GridAdapterHandle, mountGridHost } from '@eregister/wit-grid-core/internal';

// packages/react/src/WitGrid.tsx:226
...
import type { InternalColumnDef } from '@eregister/wit-grid-core/internal';
...
| Core boundary tests     | `corepack pnpm --filter @eregister/wit-grid-core exec vitest run src/boundary.test.ts`                  | exit 0               |
| Core architecture tests | `corepack pnpm --filter @eregister/wit-grid-core exec vitest run src/engine/architectureGuards.test.ts` | exit 0               |
| Core full tests         | `corepack pnpm --filter @eregister/wit-grid-core test`                                                  | all core tests pass  |
| React full tests        | `corepack pnpm --filter @eregister/wit-grid-react test`                                                 | all React tests pass |
...
- No behavior changes to scrolling, selection, grouping, editing, portals, or
  custom renderers.
- No public `@eregister/wit-grid-core` API additions unless they are type-only exports
...
- `packages/react/src/WitGrid.tsx` should still import `mountGridHost`,
  `GridHostWithAdapter`, and `GridAdapterHandle` from
  `@eregister/wit-grid-core/internal`, but those names must now come from the narrowed
  export list.
- `packages/react/src/WitGrid.tsx` should avoid direct dependency on a broad
...
classes to be exported from `@eregister/wit-grid-core/internal`.
...
Keep existing public-entry tests that verify `@eregister/wit-grid-core` does not expose
internal renderer/store symbols.

**Verify**:

- `corepack pnpm --filter @eregister/wit-grid-core exec vitest run src/boundary.test.ts --reporter=verbose` -> exit 0.
...
- `packages/react/src/WitGrid.tsx` and `packages/react/src/GridPortal.tsx` do
  not import any renderer classes, `GridStore`, `GridEngine`, or
  `getStoreFromApi`.

Avoid brittle checks that forbid all use of `@eregister/wit-grid-core/internal` in React;
...
If a test import used `@eregister/wit-grid-core/internal` only because it was convenient,
...
- Update `packages/core/src/boundary.test.ts` to define the new intended
  runtime export contract for `@eregister/wit-grid-core/internal`.
...
- Run existing React tests to confirm `WitGrid` and portal behavior still
...
- [ ] `@eregister/wit-grid-core/internal` exports the adapter host contract and does not
      export raw store, engine, model, or renderer classes.
- [ ] React imports from `@eregister/wit-grid-core/internal` are limited to the adapter
...
- Sealing `internal.ts` appears to require changing public `@eregister/wit-grid-core`
  application APIs.
...
- Reviewers should treat `@eregister/wit-grid-core/internal` as the adapter SDK. New
